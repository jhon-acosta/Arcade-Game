...
import {Platform,StyleSheet,Text,View,Modal,TouchableHighlight,TouchableOpacity} from 'react-native';
...
export default class Ahorcado extends Component {
  ...
  getPalabraAdivinar(){
    let PALABRAS = ["CAMPANAS", "ADORNADO", "CARAMELO"];
  ...
  componentDidUpdate(){
    ...
    if(this.state.palabraAAdivinar.length == this.state.numAciertos){
      this.setState({
        modalVisible: true,
        resultado: '¡Lo salvaste por esta vez!',
        numAciertos: 0
      });
    }
  }
        ...
        <View style={styles.modalContainer}>
          <View style={styles.innerModalContainer}>
            ...
            <TouchableHighlight onPress={() => { this.reiniciar(); }}
                                style={styles.button}>
              <Text style={styles.textoBoton}>Siguiente acertijo</Text>
            </TouchableHighlight>
          </View>                           
        </View>
      </Modal>
